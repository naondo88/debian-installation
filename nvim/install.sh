...
echo_info "Updating the mason-dependencies.txt file with installed Mason packages..."

# List Mason installed packages using headless mode and update the mason-dependencies.txt file
nvim --headless "+Mason" +q | grep -oP '(?<=└─ ).*' >"$MASON_DEP_FILE"
...
echo_success "mason-dependencies.txt has been successfully updated with the currently installed Mason packages!"
